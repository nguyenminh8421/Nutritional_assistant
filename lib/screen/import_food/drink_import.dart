import 'package:demo_1/controller/drink_controller.dart';
...
class DrinkImport extends StatelessWidget {
  final FoodDetailController controller = Get.put(FoodDetailController());
  final DrinkController drinkController = Get.find<DrinkController>();
            ...
            children: [
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    errorText: controller.quantity.value == null || controller.quantity.value <= 0
              ...
              Row(
                ...
                children: [
                  ElevatedButton(
                    onPressed: () async {
                      int? mealId = drinkController.drinkId.value;
                      int foodId = food['id'];
                      ...
                      if (mealId != null && foodId != null && quantity > 0) {
                        ...
                        showDialog(
                          ...
                          builder: (context) => AlertDialog(
                            title: Text('Lỗi'),
                            content: Text('Thông tin mealId, foodId hoặc quantity không hợp lệ. Vui lòng kiểm tra lại.'),
                            actions: [
                              TextButton(
                                onPressed: () {
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      int? mealId = drinkController.drinkId.value;
                      int foodId = food['id'];
                      double amount = controller.quantity.value;
        
                      if (mealId != null && foodId != null && amount > 0) {
                        int insertedId = await UsersDatabaseHelper().insertMealFood(mealId, foodId, amount);
                        if (insertedId != null && insertedId > 0) {
                          print('Thêm dữ liệu vào meal_food thành công, ID: $insertedId số lượng $amount ');
                          await db.updateMealTotals(mealId);
                        } else {
                          print('Thêm dữ liệu vào meal_food thất bại');
                        }

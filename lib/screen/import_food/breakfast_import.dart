...
class BreakImport extends StatelessWidget {
  final FoodDetailController controller = Get.put(FoodDetailController());
  final BreakfastController breakfastController = Get.find<BreakfastController>();
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ElevatedButton(
                    onPressed: () async {
                      int? mealId = breakfastController.mealId.value;
                      int foodId = food['id'];
                  ...
                  ElevatedButton(
                    onPressed: () async{
                      ...
                      if (mealId != null && foodId != null && quantity > 0) {
                        int insertedId = await UsersDatabaseHelper().insertMealFood(mealId, foodId, quantity);
                        if (insertedId != null && insertedId > 0) {
                          print('Thêm dữ liệu vào meal_food thành công, ID: $insertedId số lượng $quantity');
                          await db.updateMealTotals( mealId);
                          print('Thêm dữ liệu vào meal_food thành công, ');
                        } else {
                          print('Thêm dữ liệu vào meal_food thất bại');
